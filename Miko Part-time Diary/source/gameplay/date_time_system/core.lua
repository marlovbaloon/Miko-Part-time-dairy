...
-- [StrictLua] Uses typed float/integer declarations for key time variables.
...
local gt = structs.gt
float REAL_SECONDS_PER_GAME_MINUTE = state.REAL_SECONDS_PER_GAME_MINUTE
...
function core.update(dt)
    ...
    integer total_day_minutes = (gt.end_hour - gt.start_hour) * 60
    float current_minutes     = (gt.hour - gt.start_hour) * 60 + gt.minute + (gt.second / 60.0)
